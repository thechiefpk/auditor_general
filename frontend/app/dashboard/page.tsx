...
export default function DashboardPage() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      change: stats.activeScans > 0 ? 'Running now' : 'Idle',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      {/* Recent Reports Table - Compact */}
      <div className="bg-zinc-900/40 backdrop-blur-sm border border-zinc-800 rounded-lg overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-zinc-800 bg-zinc-900/20">
            <h2 className="text-sm font-semibold text-white tracking-tight uppercase">Recent Activity</h2>
            <Link
        ...
        <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
                <thead>
                    <tr className="bg-white/[0.02] border-b border-zinc-800 text-xs text-zinc-500 uppercase">
                ...
                <tbody className="divide-y divide-zinc-800/50 text-sm">
                    {stats.recentReports.length === 0 ? (
                        ...
                        stats.recentReports.map((report) => (
                            <tr key={report.reportId} className="hover:bg-white/[0.02] transition-colors">
                                <td className="px-4 py-3 font-medium text-zinc-300 font-mono">
